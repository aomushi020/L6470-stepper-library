...
// config commands
void L6470::setConfig(uint16_t _L6470_payload) {
    SPI.beginTransaction(_spiSettings);
    _spiwrite(L6470_ADD_CONFIG);
    _spiwrite(_L6470_payload >> 8);
    _spiwrite(_L6470_payload);
    SPI.endTransaction();
}

void L6470::setMaxSpeed(uint16_t _L6470_payload) {
...
void L6470::setHoldVoltage(uint8_t _L6470_payload) {
    SPI.beginTransaction(_spiSettings);
    _spiwrite(L6470_ADD_KVAL_HOLD);
    _spiwrite(_L6470_payload);
    SPI.endTransaction();
}
void L6470::setRunVoltage(uint8_t _L6470_payload) {
    SPI.beginTransaction(_spiSettings);
    _spiwrite(L6470_ADD_KVAL_RUN);
    _spiwrite(_L6470_payload);
    SPI.endTransaction();
}
void L6470::setAccelVoltage(uint8_t _L6470_payload) {
    SPI.beginTransaction(_spiSettings);
    _spiwrite(L6470_ADD_KVAL_ACC);
    _spiwrite(_L6470_payload);
    SPI.endTransaction();
}
void L6470::setDecelVoltage(uint8_t _L6470_payload) {
    SPI.beginTransaction(_spiSettings);
    _spiwrite(L6470_ADD_KVAL_DEC);
    _spiwrite(_L6470_payload);
    SPI.endTransaction();
}
void L6470::setSteps(uint8_t _L6470_payload) {
    SPI.beginTransaction(_spiSettings);
...
// Rotate commands
void L6470::run(uint8_t _dir, unsigned long _spd) {
...
void L6470::move(uint8_t _dir, unsigned long _steps) {
    _dir = constrain(_dir, 0x00, 0x01);
...
// private member
void L6470::_spiwrite(uint8_t _L6470_payload) {
